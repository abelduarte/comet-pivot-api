...
const BASE_PATH = "/api/comet-pivot";
...
export function createServer() {
  ...
  function leaderboardFor(playerId) {
    const sorted = [...entries].sort((a, b) => b.score - a.score || b.streak - a.streak || a.submittedAt.localeCompare(b.submittedAt)).slice(0, 25);
    return {
      playerId,
      entries: sorted,
  ...
  return http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && path === "/health") {
      json(res, 200, { ok: true, service: "comet-pivot-api" });
      return;
    }
    ...
    if (req.method === "POST" && path === "/scores") {
      try {
        ...
        entries.push({
          id: randomUUID(),
          playerId,
          playerName: cleanString(body.playerName, "Comet Pilot", 32),
          score,
          streak,
          secondsSurvived,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  createServer().listen(PORT, "0.0.0.0", () => {
    console.log(`comet-pivot-api listening on ${PORT}`);
  });
}
