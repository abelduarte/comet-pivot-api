...
async function withServer(fn) {
  ...
  try {
    await fn(`http://127.0.0.1:${port}/api/comet-pivot`);
  } finally {
...
test("populated board can omit current player best for a fresh viewer", async () => {
  await withServer(async baseURL => {
    await fetch(`${baseURL}/scores`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        playerId: "existing-pilot",
        playerName: "Nova",
        score: 640,
        streak: 12,
        secondsSurvived: 55
    ...
    const response = await fetch(`${baseURL}/leaderboard?playerId=fresh-viewer`);
    const body = await response.json();
    assert.equal(response.status, 200);
